...
class SpawnCreep extends ActionNode {
  ...
  tick (context) {
    ...
    const getBodyCosts = body => _.reduce(_.map(body, bodyPart => BODYPART_COST[bodyPart]), (a, b) => a + b, 0)

    var baseBody = this.params.baseBody || []
    var scaleBody = this.params.scaleBody || []
    var baseCost = getBodyCosts(getBodyCosts)
    var scaleCost = getBodyCosts(scaleBody)
    var size = Math.min(Math.floor((room.energyCapacityAvailable - baseCost) / scaleCost), fib(existingCreeps)) - 1
    var body = _.clone(baseBody)
