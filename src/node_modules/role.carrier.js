module.exports = {
  ...
  spawn: function(desired) {
    ...
    if (active.length < desired ) {
      ...
      let newCreep = spawnPoint.createCreep(dna, '', {role: 'carrier'});
      ...
      if (!creep.memory.target) {
        ...
        creep.memory.target = Game.rooms[creep.room.name].find(FIND_DROPPED_ENERGY,{
          filter: (drop) => {
            ...
            return drop.energy > 1000;
          }
        });
        if (creep.memory.target[0]) {
          creep.memory.target = creep.memory.target[0];
        }
        ...
        if(creep.memory.target) {
          let pickupAttempt = creep.pickup(creep.memory.target);
