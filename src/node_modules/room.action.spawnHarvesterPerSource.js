...
class SpawnHarvesterPerSource extends ActionNode {
  open (context) {
  ...
  tick (context) {
    ...
    var harvesters = room.find(FIND_MY_CREEPS, {
      filter: creep => creep.memory.behavior === 'harvester'}
    )
    ...
    _.each(sources, source => {
      ...
      if (inDanger) {
        this.log(context, `Source ${source.id} is in danger, don't send harvester`)
        return
      }

      var hasHarvester = false
      _.each(harvesters, harvester => {
        if (harvester.memory.digSource === source.id) {
          hasHarvester = true
        }
      })

      if (hasHarvester) {
        this.log(context, `Source ${source.id} already has a harvester assigned`)
        return
      }
      ...
      this.log(context, spawns[0].createCreep(
        [WORK, CARRY, MOVE],
        null,
        {behavior: 'harvester', digSource: source.id}
      ))
    })
...
module.exports = SpawnHarvesterPerSource
